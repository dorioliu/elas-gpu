...
Matrix::Matrix (const int32_t m_,const int32_t n_,const FLOAT_t* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
void Matrix::getData(FLOAT_t* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(FLOAT_t s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(FLOAT_t s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const FLOAT_t&angle) {
    FLOAT_t s = sin(angle);
    FLOAT_t c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const FLOAT_t&angle) {
    FLOAT_t s = sin(angle);
    FLOAT_t c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const FLOAT_t&angle) {
    FLOAT_t s = sin(angle);
    FLOAT_t c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
bool Matrix::lu(int32_t *idx, FLOAT_t &d, FLOAT_t eps) {
  ...
  FLOAT_t* vv = (FLOAT_t*)malloc(n*sizeof(FLOAT_t)); // vv stores the implicit scaling of each row.
  d = 1.0;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((FLOAT_t)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
