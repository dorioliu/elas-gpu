...
//typedef float  FLOAT_t;    // single precision
...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const FLOAT_t* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(FLOAT_t* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(FLOAT_t s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(FLOAT_t s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const FLOAT_t&angle);
  static Matrix rotMatY(const FLOAT_t&angle);
  static Matrix rotMatZ(const FLOAT_t&angle);
  ...
  FLOAT_t  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M, FLOAT_t eps=1e-20);            // solve linear system M*x=B, replaces *this and M
